...
final class Availability extends AbstractSection {
	...
	public function run(Context $ctx, array $o): SectionResult {
		$items = $ctx->items(['tags' => [], 'keys' => $o['keys'], 'names' => '']);
		...
		$no_data = count(array_diff_key($ctx->hosts, $per_host));
		$below = array_filter($per_host, static fn($r) => $r['availability'] < $target);
		$met = count($per_host) - count($below);
		$mean = $per_host ? array_sum(array_column($per_host, 'availability')) / count($per_host) : null;
		...
		$result = (new SectionResult())->kpis([
			...
			['label' => sprintf('Devices at or above %s%%', self::num($target)), 'value' => $met, 'format' => 'int',
				'hint' => sprintf('of %d measured', count($per_host))],
		...
		uasort($list, static fn($a, $b) => [$a['availability'], $a['host']] <=> [$b['availability'], $b['host']]);
